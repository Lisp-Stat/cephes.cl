...
/*
 * Note: Scipy was using 'gamma', and renamed Cephes' version to
 * 'Gamma'. I could not find a C version of Scipy's 'gamma', so we
 * have two choices: use the internal Gamma, as Cephes does, or use
 * the libc tgamma. We use Gamma. */
  ...
  double a, b, c, x;
{
 ...
 hypdon:
  if (err > ETHRESH) {
    mtherr( "hyp2f1", TLOSS ); /* Scipy does not distinguish between TLOSS and PLOSS  */
		       ...
		       double *loss)
{
  ...
  *loss = 0;
  ...
  if (fabs(da) > MAX_ITERATIONS) {
    ...
    mtherr( "hyp2f1", TLOSS ); /* Cephes doesn't have a 'no result' return */
